...
for i = 1:length(n_values)
    ...
    %----------------------calcualtions for C1--------------------------------------------
        ...
        elseif (r >= r0_C1) && (C1+B1/2 >= B1) && (r*sin(C1-B1/2) < d)%----caseD
            %Auncovered1 = r*sin(C1-B1/2)*r*cos(C1-B1/2)*n;
            Auncovered1 = (L1+L0_C1)*d + Lr*d;
            Acovered_C1 = (L2_C1+L1+L0_C1)*d*n - Auncovered1;
            coverage_values_1(i) = Acovered_C1 / (L*d);
            str1 = 'caseA';
    ...
    %----------------------calcualtions for C2---------------------------------------------
        ...
        elseif (L1 < 0) && (r0_C2 < r)%------------------------------------caseB
            Auncovered = (r0_C2-(L1/2)/cos(B1/2))*(d/2-(abs(L1)/2)*tan(B1/2))*n;
    ...
    elseif L1 < 0
        ...
        elseif strcmp(str1, 'caseA')||strcmp(str1, 'caseB')%----caseD
            ...
            Roverlap = abs(2*(h*h*sin(angle1)*sin(angle2))/(2*sin(angle1+angle2)));
...
figure('Name', 'Coverage Comparison', 'Position', [180, 180, 1000, 600]);
bar(n_values, [coverage_values_1; coverage_values_2; coverage_values_3; coverage_values_4] * 100, 0.6);
ylim([0 100]);
...
ylabel('Coverage (%)');
title('Coverage Comparison of Three Configurations and Real Radar');




% Define custom colors for each bar
colors = [0.1 0.2 0.6;  % Configuration 1 (blue)
          0.6 0.1 0.1;  % Configuration 2 (red)
          0.1 0.6 0.1;  % Configuration 3 (green)
          0.8 0.5 0];    % Real Radar (orange)

% Define characters for each configuration
characters = {'Configration1', 'Configration2', 'Configration3', 'Radar One'};

% Plot the radar density comparison
figure('Name', 'Radar Density and Wastage Comparison', 'Position', [180, 180, 1000, 500]);
subplot(1, 2, 1);
h = bar(1 , [density1; density2; density3; density4], 0.6, 'FaceColor', 'flat');
colormap(colors); % Set custom colormap
ylim([0, max([density1; density2; density3; density4])]); % Adjust y-axis limit

% Add text labels on top of each bar
for i = 1:numel(h)
    text(h(i).XEndPoints, h(i).YEndPoints, sprintf('%s\n%s', characters{i}, num2str(h(i).YData')), ...
        'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom');
end


% Set legend and labels
legend({'Configuration 1','Configuration 2','Configuration 3','Radar One'}, 'Location', 'northwest');
xlabel('Configurations');
ylabel('Radar Density (Number of Radar per KM)');
title('Radar Density Comparison (How many radar needed to cover 1 KM road)');
ylim([0 3.5]);


% Plot the wastage area comparison
subplot(1, 2, 2);
plot(n_values, wastage/ref * 100, '-*m', 'LineWidth', 1.5);
hold on;
plot(n_values, wastage1/ref * 100, '-^r', 'LineWidth', 1.5);
plot(n_values, wastage2/ref * 100, '-*b', 'LineWidth', 1.5);
plot(n_values, wastage3/ref * 100, '-*g', 'LineWidth', 1.5);
hold off;
ylim([0 300]);
...
xlabel('Number of Radar Pairs (n)');
ylabel('Wastage Area (%)');
legend('Radar One', 'Configuration 1', 'Configuration 2', 'Configuration 3', 'Location', 'northwest');
title('Wastage Area Comparison');

% Plot the blindspot area comparison
figure('Name', 'Wastage Area Comparison', 'Position', [180, 180, 800, 500]);
plot(n_values, blindspot/ref * 100, '-*m', 'LineWidth', 1.5);
hold on;
plot(n_values, blindspot1/ref * 100, '-*r', 'LineWidth', 1.5);
plot(n_values, blindspot2/ref * 100, '-*b', 'LineWidth', 1.5);
plot(n_values, blindspot3/ref * 100, '-^g', 'LineWidth', 1.5);
hold off;
ylim([0 100]);

% Set legend and labels
xlabel('Number of Radar Pairs (n)');
ylabel('Blindspot Area (%)');
legend('Radar One', 'Configuration 1', 'Configuration 2', 'Configuration 3', 'Location', 'northwest');
title('Blindspot Area Comparison');
